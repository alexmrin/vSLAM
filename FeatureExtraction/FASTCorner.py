...
def fast_corners(grayframe, threshold, diffpixelnum):
    ...
    for i in range(3, height - 3):
        for j in range(3, width - 3):
            ...
            #Bresenham circle patttern
            patterns = [grayframe[i - 3, j - 1], grayframe[i - 3, j], grayframe[i - 3, j + 1],
            grayframe[i - 2, j + 2], grayframe[i - 1, j + 3], grayframe[i, j + 3],
            grayframe[i + 1, j + 3], grayframe[i + 2, j + 2], grayframe[i + 3, j + 1],
            grayframe[i + 3, j], grayframe[i + 3, j - 1], grayframe[i + 2, j - 2],
            grayframe[i + 1, j - 3], grayframe[i, j - 3], grayframe[i - 1, j - 3],
            grayframe[i - 2, j - 2]]
